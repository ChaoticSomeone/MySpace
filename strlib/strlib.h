...
#if defined(_WIN32) && !defined(__linux__) // Windows variant

// Function Prototypes
// Total amount of functions in strlib.h: 47 (47 defined)
int StrLength(str a);
...
str StrMerge(str a, str b){
    str newStr = (str) calloc(StrLength(a)+StrLength(b)+1, 1);
    int idx = 0;
...
str StrReplaceAll(str base, str what, str with){
    size_t size = StrLength(base)- StrLength(what) * StrCount(base, what) + StrLength(with) * StrCount(base, what);
    str newStr = (str) calloc(size + 1,1);
    while (StrFind(base, what)){
        newStr = StrMerge(newStr, StrReplaceFirst(StrSplitAt(base, StrSearchFirst(base, what)[1] + 1, 0), what, with));
        base += StrSearchFirst(base, what)[1] + 1;
    }
    newStr[size - StrLength(base)] = 0;
    newStr = StrMerge(newStr, base);
    return newStr;
}
 ...
 */
int IsTitle(str a){
    return StrMatch(a, ToTitle(a)) && !(IsNumeric(a) || IsSymbolic(a));
}
...
str StrInsertAt(str base, int idx, str what){
    if (idx == StrLength(base)){
        return StrMerge(base, what);
    }
    if (idx == 0){
        return StrMerge(what, base);
    }
    str prevChar = (str) calloc(2,1);
    prevChar[0] = base[idx];
    str prevHalf = StrSplitAt(base, idx, 0);
    str newStr = StrMerge(prevHalf, what);
    newStr = StrMerge(newStr, prevChar);
    if (idx < StrLength(base)-1){
        str nextHalf = StrSplitAt(base, idx, 1);
        newStr = StrMerge(newStr, nextHalf);
    }
    return newStr;
}
...
str BinaryAdd(str a, str b){
    ...
    strncpy(newA, a, StrLength(a));
    strncpy(newB, b, StrLength(b));
    while (StrLength(newA) < StrLength(newB)){newA = StrInsertAt(newA, 0, "0");}
    while (StrLength(newB) < StrLength(newA)){newB = StrInsertAt(newB, 0, "0");}
    str newStr = (str) calloc(1,1);
    char carry = 0;
    char sum = 0;
    for (int i = StrLength(a)-1; i >= 0; i--){
        char bitA = newA[i] - 48;
        char bitB = newB[i] - 48;
        char a_b = bitA && !bitB || bitB && !bitA;
        sum = a_b && !carry || carry && !a_b;
        carry = carry && a_b || bitA && bitB;
        char newBit[] = {sum+48, 0};
        newStr = StrInsertAt(newStr, 0, newBit);
    }
    return newStr;
}

/**
 * Converts the decimal (base 10) number 'dec' into a binary string (base 2)
 * @param dec ... integer to convert
 * @param signMode ... what sign representation to use
 * @return 'dec' as a binary string
 */
str Dec2Bin(int dec, int signMode){
    int dec2 = abs(dec);
    int size = ceil(log2(dec2+1));
    if (signMode == SIGNMODE_S2C && dec < 0){
        size *= 2;
    }
    str bin = (str) calloc(size+1, 1);
    if (signMode < SIGNMODE_S2C || signMode == SIGNMODE_S2C && dec > 0){
        for (int i = 0; dec2 > 0; i++){
            bin[i] = 48 + (dec2 % 2);
            dec2 /= 2;
        }
    }
    ...
    if (signMode == SIGNMODE_SBIT){
        bin = StrInsertAt(bin, 0, dec < 0 ? "1" : "0");
    }
...
#elif defined(__linux__) && !defined(_WIN32) // Linux variant
...
str Float2Str(float a){
    char sign = a < 0 ? '-' : '+';
    a *= a < 0 ? -1 : 1;
    int int_pre = (int) floorf(a);
    float float_post = a - int_pre;
    int preLen = (int) ceilf(log10f(int_pre));
    str pre = (str) calloc(preLen + 1, 1);
    ...
    str revPre = (str) calloc(preLen + 1, 1);
    revPre = StrReverse(pre);
    pre = revPre;
    int postLen = 0;
    float float_post2 = float_post;
    while (float_post2 != 0){
        float_post2 *= 10;
        int res = ((int) floorf(float_post2)) % 10;
        float_post2 -= res;
        postLen++;
    }
    str post = (str) calloc(postLen + 1, 1);
    for (int i = 0; float_post != 0; i++){
        float_post *= 10;
        int res = ((int) floorf(float_post)) % 10;
        float_post -= res;
        post[i] = 48 + res;
    }
    str newStr = (str) calloc(2, 1);
    if (sign == '-'){
        newStr[0] = '-';
    }
    newStr = StrMerge(newStr, pre);
    newStr = StrMerge(newStr, ".");
    newStr = StrMerge(newStr, post);
    return newStr;
}
